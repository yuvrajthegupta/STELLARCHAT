...
export class AddressBook {
    ...
    loadedConfig;
    deploymentsFilePath;
    constructor(ids, hashes, fileName, loadedConfig) {
        this.ids = ids;
        this.hashes = hashes;
     ...
     */
    static loadFromFile(network, loadedConfig, folder = '.soroban') {
        ...
        try {
            ...
            return new AddressBook(new Map(Object.entries(contractObj.ids)), new Map(Object.entries(contractObj.hashes)), fileName, loadedConfig);
        }
        catch {
    ...
    updateDeployments(contractkey) {
        var updated = false;
        this.deployments.forEach((deploymentInfo) => {
    ...
    writeToFile() {
        ...
        const newDeploymentFile = JSON.stringify(this.deployments, (key, value) => {
            if (value instanceof Map) {
                return Object.fromEntries(value);
            }
            else {
